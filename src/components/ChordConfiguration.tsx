...
function ChordConfiguration(props: ChordConfigurationProps) {
    ...
    useEffect(() => {
        if (props.note !== "" && props.type !== "") {
            ...
            diagramWrapper.current && new ChordBox(`#chord-diagram-wrapper-${props.index}`, options).draw({
    ...
    return (
        <ChordConfigurationWrapper>
            <StyledButton onClick={() => props.removeChord(props.index)}>-</StyledButton>
            {props.index + 1}.
            <StyledSelectWrapper>
                Note:
            ...
            <StyledSelectWrapper>
                ...
                <StyledSelect value={props.type} width={80} onChange={handleTypeChange}>
                    <option value={""} key={"null"}>--</option>
                    {Chords[props.note] && Object.keys(Chords[props.note]).map(type => (
                        <option value={type} key={type}>{type}</option>
                    ))}
                </StyledSelect>
            </StyledSelectWrapper>
            <ChordDiagramWrapper id={`chord-diagram-wrapper-${props.index}`} ref={diagramWrapper} debugMode={true} />
        </ChordConfigurationWrapper>
    );
}
